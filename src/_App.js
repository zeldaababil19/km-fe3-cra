...
function App() {
  ...
  return (
    <div className="App">
      ...
      <div id="track">
        {trackData.map((track) => (
          <Track key={track.id} name={track.name} artist={track.artists[0].name} album={track.album.name} buttonName="Go To Spotify" uri={track.external_urls.spotify} />
        ))}
      </div>
    </div>
  );
